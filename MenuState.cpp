...
", p_game->fonts()->get("about_font"), 20);
    m_aboutText.setPosition(50, WINDOW_HEIGHT - 350);

    m_backText.setPosition(5, 10);

//    m_pointer.setTexture(p_game->textures()->get("ball"));
...
void MenuState::setPointerPosition() {
    sf::Vector2f textPosition = m_options[m_selected].getPosition();
    unsigned textWidth = m_options[m_selected].getLocalBounds().width;

    m_pointer.setPosition(textPosition.x + textWidth + 30, textPosition.y + m_pointer.getGlobalBounds().height/2);
}
